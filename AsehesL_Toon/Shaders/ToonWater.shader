Shader "Unlit/ToonWater"
{
	Properties
	{
		...
		_Foam ("Foam", 2D) = "white" {}
		_InteractFoam ("InteractFoam", 2D) = "white" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed4 refcol = tex2D(_ReflRamp, half2(rdn, rdn));

				fixed4 wcol = tex2D(_WaterRamp, half2(depth, depth)) * 0.5 + refcol * 0.5;

				float depthParam = pow(1 - depth, 3) + interact * 2;
				//float depthParam = (1 - depth) + interact * 2;

				half2 uvfoam = TRANSFORM_TEX(i.uv, _Foam);

				fixed foam = (tex2D(_Foam, uvfoam + half2(_Speed.x, _Speed.y) * _Time.y) 
				+ tex2D(_Foam, half2(uvfoam.x + _Speed.x * _Time.y, 1 - uvfoam.y + _Speed.y * _Time.y))).r * 0.5 * depthParam 
				+ depthParam * 0.35;
